...
def acc_estimator(y_true:np.ndarray, y_pred:np.ndarray)->float:
    correct = y_true == y_pred
    return sum(correct) / len(correct)

def acc_estimator2(y_true:np.ndarray, y_pred:np.ndarray)->float:
    y_pred = acc_estimator(y_true, y_pred)
    mask = y_pred == np.max(y_pred)
    y_pred[mask] = 1
    y_pred[~mask] = -1
